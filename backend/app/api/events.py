...
from ..models import (
    BudgetItem, Contact, Expense, ExpenseSheet, Message, Risk, Role, Task, TaskCategory,
    TaskItem, TaskPriority, User,
)
...
async def _fingerprints(session) -> dict[int | None, str]:
    ...
    merge((await session.execute(
        select(
            TaskItem.workspace_id, func.count(),
            func.sum(case((TaskItem.done.is_(True), 1), else_=0)),
            func.coalesce(func.sum(func.length(TaskItem.text)), 0),
        ).group_by(TaskItem.workspace_id)
    )).all())
    # довідники: додали/перейменували/видалили розділ чи рівень важливості
    merge((await session.execute(
        select(
            TaskCategory.workspace_id, func.count(),
            func.coalesce(func.sum(func.length(TaskCategory.label)), 0),
        ).group_by(TaskCategory.workspace_id)
    )).all())
    merge((await session.execute(
        select(
            TaskPriority.workspace_id, func.count(),
    ...
    # записник: додали/поправили/видалили контакт
    merge((await session.execute(
        select(
            Contact.workspace_id, func.max(Contact.id), func.count(),
            func.coalesce(func.sum(func.length(Contact.name)), 0),
            func.coalesce(func.sum(func.length(Contact.note)), 0),
            func.sum(case((Contact.deleted_at.is_not(None), 1), else_=0)),
